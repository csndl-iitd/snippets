...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='D:\\Github\\openbci-brainflow\\test.py',
...
# END BrainFlow Code -----------------------------------------------------------
    ...
    for thisTrial in trials:
        ...
        trial = data.Routine(
            name='trial',
            components=[fixation, arrow, resp],
        )
...
# END BrainFlow Code -----------------------------------------------------------
            ...
            if arrow.status == NOT_STARTED and tThisFlip >= 1.2-frameTolerance:
                # keep track of start time/frame for later
...
# END BrainFlow Code -----------------------------------------------------------
            ...
            if arrow.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > arrow.tStartRefresh + 0.8-frameTolerance:
                    # keep track of stop time/frame for later
...
# END BrainFlow Code -----------------------------------------------------------
            ...
            if resp.status == NOT_STARTED and tThisFlip >= 1.2-frameTolerance:
                # keep track of start time/frame for later
            ...
            if resp.status == STARTED and not waitOnFlip:
                theseKeys = resp.getKeys(keyList=['a', 'l'], ignoreKeys=["escape"], waitRelease=False)
...
# BrainFlow Code ---------------------------------------------------------------
    ...
    filename = thisExp.dataFileName
    data.to_csv(filename+'_eeg.csv')
    data.to_pickle(filename+'_eeg.pkl')
# END BrainFlow Code -----------------------------------------------------------
